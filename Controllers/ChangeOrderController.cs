...
public class ChangeOrderController : Controller
{
  ...
  public async Task<IActionResult> Index(int? id_project)
  {
   ...
   // Verificamos si viene un nuevo valor desde el filtro
    ...
    var changeQuery = _context.ChangeOrders.Include(p => p.project).AsQueryable();

    // Aplica el filtro si se seleccionó un proyecto
    if (id_project.HasValue)
    {
        changeQuery = changeQuery.Where(p => p.id_project == id_project.Value);
    }
    ...
    // Ejecuta la consulta
    var changeOrder = await changeQuery.ToListAsync();
